...
| 项 | 竖屏 | 横屏 | 理由 |
|---|---|---|---|
| 最底下的方案 / 表情 / 剪贴板 / 编辑 / 麦克风行 | 有 | 隐藏（`hide_in_landscape`） | 屏高只有 400dp，这一行让给键盘；emoji 从 `123` 上滑的「更多」面板进，剪贴板和编辑面板在工具栏上也有 |
...
| 切方案 | 左下角键点一下弹出方案选单（双拼 / 九宫格…），长按选系统输入法 | 位置同 iOS 地球键；选单比轮换直接 |
