...
| 定稿后开了 LLM 纠错（且文字不短于阈值） | 进入「纠错中」，最多等 6 秒 | 原文留作待定文字，纠错回来整段替换后上屏 |
| 纠错中按下别的键、再点麦克风键、光标被点走、键盘收起、换输入框 | 立刻结束，取消纠错请求 | 原文按当前样子上屏 |
| 纠错失败或超时 | 结束，胶囊显示错误 2.5 秒 | 原文上屏 |
| 纠错结果没通过校验（空、长短差太多、带说明、被翻译） | 结束，不提示 | 原文上屏 |

规则只有一条：**识别出来的字永远保留**，要么等定稿，要么按当前样子上屏，从不丢弃，也不留下半截待定文字。
识别中的文字是输入框里带下划线的待定文字（`setComposingText`），中间结果也经过词库映射。
纠错也守这条：纠错期间原文已经在输入框里，只有及时回来、又通过校验的纠错结果才会替换它。

**热词**：开始听写时把词库（`voice_vocab.yaml`）的 `hotwords` 和映射规则的目标词合在一起，大小写不敏感去重、保持顺序，
跳过纯标点和超过 20 字的，最多 100 个，放进火山请求的 `context`；设置里填了云端热词表 ID 时改用 `corpus.boosting_table_id`，不再内联。

**LLM 纠错**（设置 → 语音输入 → LLM 纠错，默认关）：OpenAI 兼容的 Chat Completions，非流式。
服务商预置火山方舟、DeepSeek、阿里云百炼（地址固定，并关掉深度思考），也可以自定义地址（只接受 https，本机和局域网除外）；
模型名、提示词（留空用内置的最小幅度纠错提示词）、短文本阈值（默认少于 4 个字不纠）和「测试连接」都在同一组。
API Key 跟火山凭证一样只存在 `VoiceCredentialStore`，不进 Rime 用户目录、不进日志。没开或没配好时听写行为跟以前完全一样。

- 开了纠错时，会话中途定稿的句子不再分段上屏，而是一直留在待定文字里，结束时整段纠错；
- 请求：规则放系统消息，识别结果用 `<transcript>` 标签隔开放在用户消息里，前面附上热词做参考词表；
- 纠错结果先剥掉推理块、代码块、整段引号，再校验：多出 `max(8, 50%)` 或少了 `max(3, 30%)` 个字、开头是「以下是…」之类的说明、
  多出「说明：」段落、有没有汉字跟原文不一致，都退回原文；通过的结果再过一遍词库映射；
- debug 包「开发者 → 模拟纠错」用假纠错（1.2 秒后补空格和句号，或者失败、或者一直不回），不连网络也能看各个状态。
- 标点设置把句末句号去掉时（空格、不加、去掉句末句号等），接着上一次听写说：光标前还是上次上屏的那段字、同一个输入框、没选中文字，就在这次第一段前面补上句号本该变成的分隔符（`PunctuationFormatter.separatorAfter`）；打过别的字、挪过光标或换了输入框就不补，默认设置也不补。

**胶囊**：强调色底、实心麦克风加三根随音量起伏的竖条，出错时换成划掉的麦克风加文字。
纠错中换成魔棒图标加「纠错中」，三根竖条轮流冒尖（同样的尺寸 token，不另设）。
